...
function StandingsTable({
  ...
  if (isLoading) {
    return (
      <div className="neon-card rounded-2xl p-8 text-center">
        <Loader2 className="w-6 h-6 animate-spin mx-auto text-[#22D3EE]" />
      </div>
    )
  }
  ...
  return (
    <div className="neon-card rounded-2xl overflow-hidden">
      <div
        className="flex items-center gap-3 px-4 py-3 border-b border-[#22D3EE]/10 text-xs font-medium text-muted-foreground uppercase"
        style={{
          background:
      ...
      <div className="p-1">
        {standings.slice(0, 8).map((standing) => (
          <div
            ...
            className={`flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-[#22D3EE]/5 transition-all ${
          ...
          >
            ...
            <div className="flex-1 flex items-center gap-2 min-w-0">
              ...
              <span className="text-sm font-medium truncate group-hover:text-[#22D3EE] transition-colors">
                {standing.team.name}
              </span>
            </div>

            <span className="w-8 text-center text-sm text-muted-foreground hidden sm:block">
              {standing.played}
            </span>
            <span className="w-8 text-center text-sm text-[#10B981] hidden sm:block">
              {standing.won}
            </span>
            <span className="w-8 text-center text-sm text-[#FBBF24] hidden sm:block">
              {standing.drawn}
            </span>
            <span className="w-8 text-center text-sm text-[#EF4444] hidden sm:block">
              {standing.lost}
            </span>
            ...
            <span
              className="w-10 text-right text-sm font-bold text-[#22D3EE]"
      ...
      <Link
        href="/standings"
        className="block text-center py-3 text-sm text-[#22D3EE] hover:text-[#00E07A] border-t border-[#22D3EE]/10 transition-colors"
      >
        Tam Tabloyu Gör
      </Link>
    </div>
      ...
      <div className="min-h-screen">
        <main className="w-full px-4 py-12 text-center">
          ...
          <p className="text-muted-foreground mt-2">
            Geçersiz lig kodu: {code}
          </p>
          <Link
            href="/standings"
            className="text-[#22D3EE] hover:text-[#00E07A] mt-4 inline-block transition-colors"
          >
            Tüm Liglere Git
          </Link>
        </main>
      ...
      <main className="w-full px-4 pb-12">
        ...
        <div className="relative py-12 lg:py-16">
          ...
          <div className="relative flex items-center gap-6">
            ...
            <div>
              <h1
                className="text-4xl md:text-5xl font-extrabold tracking-tight bg-gradient-to-r from-[#00E07A] via-[#22D3EE] to-[#FBBF24] bg-clip-text text-transparent"
                style={{
            ...
            <button className="ml-auto p-3 rounded-xl glass-card hover:bg-muted/50 transition-all border border-[#FBBF24]/20 hover:border-[#FBBF24]/40 hover:shadow-neon-gold">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-8">
            {[
              ...
              },
              {
                icon: TrendingUp,
                label: 'Toplam Gol',
                value: standings.reduce((acc, s) => acc + s.goalsFor, 0) || '-',
                color: 'green',
              },
            ].map((stat, i) => {
              const colors = {
                blue: { text: '#00E07A', glow: 'rgba(0, 224, 122, 0.3)' },
                cyan: { text: '#22D3EE', glow: 'rgba(34, 211, 238, 0.3)' },
        ...
        <div className="grid gap-8 lg:grid-cols-3">
          ...
          <div className="lg:col-span-2 space-y-8">
            <section>
              <SectionTitle
                gradient="primary"
                description="Bu haftanın maçları"
                neonGlow
              >
                Yaklaşan Maçlar
              </SectionTitle>
            ...
            <section>
              <SectionTitle
                gradient="secondary"
                description="Son oynanan maçlar"
                neonGlow
              >
                Son Sonuçlar
              </SectionTitle>
          ...
          <div className="space-y-8">
            <section>
              <SectionTitle gradient="accent" neonGlow>
                Puan Tablosu
              </SectionTitle>
              <StandingsTable
                standings={standings}
